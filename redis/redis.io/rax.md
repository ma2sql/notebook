...
Rax는 radix tree의 구현이다. 최초로 쓰여진 / 사용되기 위해서 / 지정된 위치에서 / 레디스의 / 성능 문제를 해결하기 위해서,
그렇지만 바로 변경하했다 / 단일 프로젝트로 / 레디스가 스스로 재사용하도록 하기 위해서 / 초기 성향의 어플리케이션 외에도 / 그리고 다른 프로젝트 역시도..
...
주 목적은 찾는 것이다. 적절한 균형, 성능과 메모리 사용의 사이에서, 완전히 구현된 기능을 제공하기 위해서 / radix tree는 매우 다른 요구 사항을 콥(cope)한다.
...
* 메모리 의식
    ...
    + 노드 내에 NULL 포인터를 저장하지 않도록 한다. 만약 키가 NULL을 저장해야 한다면 (`isnull` 비트가 노드 헤더내애 있다)
    + Lack of parent node reference. A stack is used instead when needed.
    + 부모 노드 참조의 부족. 필요할 때에는 스택이 대신 사용된다.
* Fast lookups:
...
* Complete implementation:
    ...
    + 이터레이터. 반복자. (트리가 변경되는 동안 이터레이터를 사용하는 방법을 포함하는)
    ...
    + oom을 보고하고 억제하는 능력. malloc()이 NULL을 반환한다면, API는 oom 에러를 보고할 수 있고, 항상 일관적인 상태의 트리를 남겨둔다.
...
노드의 레이아웃은 다음과 같다. 이 예제에서, 키를 나타내는 노드는 `x`, `y`, `z` 3개의 자식 노드를 가진다. 다이어그램의 모든 공간은 바이트로 표현된다.
...
헤더 `HDR`은 실제 다음과 같은 필드를 가지는 비트필드이다. 
...
압축된 노드는 노드의 체인을 표현하고, 그 노드들은 키가 아니며, 정확히 하나의 자식 노드를 가진다. 그래서 대신 저장하는 것은:

    A -> B -> C -> [some other node]

We store a compressed node in the form:
우리는 이러한 형태로 압축된 노드를 저장한다.
