...
백그라운드에서 RDB 파일을 만들거나, AOF가 유효화되어 있는 경우에 Append Only File을 재작성하기 위해서, Redis는 백그라운드 프로세스를 포크(fork) 해야한다. (메인 스레드에서 실행되는) 포크 오퍼레이션은 그 자체로 지연을 발생시켜버릴 수도 있다.

Forking is an expensive operation on most Unix-like systems, since it involves copying a good number of objects linked to the process. 
포크는 대부분의 Unix/Linux 시스템에서는 비싼 오퍼레이션인데, 해당 프로세스와 연결된 많은 수의 오브젝트들의 복사(copy)가 포함되기 때문이다.
This is especially true for the page table associated to the virtual memory mechanism.
이것은 특별한 사실이고 가상 메모리(virtual memory) 메커니즘과 관련된 페이지 테이블에 
